		...
		if (str_scomp (varr[0], "parol") == __TRUE)
			...
			if(strstr (varr[1], "value="))
				...
				if(strstr (varr[0], "serno_"))
					...
					else if((short)(web_param_input&0x0000ffffUL))
						{
						short tempSS=(short)(web_param_input&0x0000ffffUL);
						if((tempSS>=5)&&(tempSS<=300))
							{
							lc640_write_int(EE_MNEMO_ON,mnON);
							lc640_write_int(EE_MNEMO_TIME,tempSS);
							}
						}
...
U16 cgi_func (U8 *env, U8 *buf, U16 buflen, U32 *pcgi) {
  ...
  switch (env[0]) {
			...
			case '0':
				...
				switch (env[2]) {
					case '1':
						...
						#if(UKU_VERSION==300)		
						if(AUSW_MAIN==2424)			len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/24-24А"));
						else if(AUSW_MAIN==2412)	len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/24-12А"));
						else if(AUSW_MAIN==6010)	len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/60-10А"));
						else if(AUSW_MAIN==4810)	len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/48-10А"));
						else if(AUSW_MAIN==6005)	len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/60-5А"));
						else if(AUSW_MAIN==4805)	len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/48-5А"));
						else 						len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("модель неопределена"));
						#endif //(UKU_VERSION==300)				{

						#if(UKU_VERSION==900)		
						if(AUSW_MAIN==2450)			len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/24-50А"));
						else if(AUSW_MAIN==2425)	
							{
							if(NUMIST==1)			len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/24-25А-1БПС"));
							else if(NUMIST==2)		len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/24-25А-2БПС"));
							}
						else if(AUSW_MAIN==6024)	len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/60-24А"));
						else if(AUSW_MAIN==6012)	
							{
							if(NUMIST==1)			len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/60-12А-1БПС"));
							else if(NUMIST==2)		len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/60-12А-2БПС"));
							}
						else if(AUSW_MAIN==4824)	len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/48-24А"));
						else if(AUSW_MAIN==4812)	
							{
							if(NUMIST==1)			len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/48-12А-1БПС"));
							else if(NUMIST==2)		len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/48-12А-2БПС"));
							}
						else if(AUSW_MAIN==6030)	len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/60-30А"));
						else if(AUSW_MAIN==6015)	
							{
							if(NUMIST==1)			len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/60-15А-1БПС"));
							else if(NUMIST==2)		len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/60-15А-2БПС"));
							}
						else if(AUSW_MAIN==4840)	len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/48-40А"));
						else if(AUSW_MAIN==4820)	
							{
							if(NUMIST==1)			len = sprintf((char *)buf,(const char *)&env[4],pal_cyr_coder("ИБЭП220/48-20А-1БПС"));
    ...
    case 'o':
		...
		switch (env[1]) {
			case 'u':
				len = sprintf((char *)buf,(const char *)&env[3],Uload);
		  		break;
			case 'i':
				len = sprintf((char *)buf,(const char *)&env[3],iload);
		  		break;
		}
		break;
          		...
          		switch (env[2]) {
		        	case '1':
		     		...
		     		case '5':
		          		...
		          		break;
		     		case '6':
		         		len = sprintf((char *)buf,(const char *)&env[4],MNEMO_TIME,(MNEMO_ON==mnON)?"on":"off");
		          		break;
		     		case '7':
